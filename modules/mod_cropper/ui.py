...
from core.pdf_engine import merge_pdf_with_smart_toc
from .worker import CropWorker

class CropperGraphicsView(QGraphicsView):
    # 这里直接原封不动保留你最初发给我的视图类，不修改底层交互
    ...
    def __init__(self, parent=None):
        ...
        self.pdf_doc = None; self.current_page = -1; self.zoom_factor = 1.0
        self.page_item = None; self.page_data_dict = {}; self.drag_line = None; self.RENDER_SCALE = 2.0

    def load_pdf(self, doc, target_page=0, data_dict=None):
        self.pdf_doc = doc; self.page_data_dict = data_dict if data_dict else {}
        self.current_page = -1; self.show_page(target_page)

    def show_page(self, page_num):
        if not self.pdf_doc or page_num < 0 or page_num >= len(self.pdf_doc): return
        self.current_page = page_num; page = self.pdf_doc[page_num]
        ...
        self.scene.clear(); self.page_item = self.scene.addPixmap(QPixmap.fromImage(img))
        self.page_item.setZValue(-1); self.scene.setSceneRect(QRectF(self.page_item.boundingRect()))
        self.resetTransform(); self.scale(self.zoom_factor, self.zoom_factor)
        if self.current_page not in self.page_data_dict:
            self.page_data_dict[self.current_page] = {'v_lines': [0.5], 'h_lines': [], 'disabled': []}
        self.pageChanged.emit(self.current_page); self.viewport().update()
    ...
    def drawForeground(self, painter, rect):
        ...
        w, h = self.page_item.boundingRect().width(), self.page_item.boundingRect().height()
        ...
        for vx in data['v_lines']: painter.drawLine(int(vx * w), 0, int(vx * w), int(h))
        for hy in data['h_lines']: painter.drawLine(0, int(hy * h), int(w), int(hy * h))
        ...
        if event.button() == Qt.RightButton:
            for i, vx in enumerate(data['v_lines']):
                if abs(pos.x() - vx * w) < hit_range:
                    data['v_lines'].pop(i); data['disabled'].clear(); self.viewport().update(); return
            for i, hy in enumerate(data['h_lines']):
                if abs(pos.y() - hy * h) < hit_range:
                    data['h_lines'].pop(i); data['disabled'].clear(); self.viewport().update(); return
            return
        if event.button() == Qt.LeftButton:
            for i, vx in enumerate(data['v_lines']):
                if abs(pos.x() - vx * w) < hit_range: self.drag_line = ('v', i); return
            for i, hy in enumerate(data['h_lines']):
                if abs(pos.y() - hy * h) < hit_range: self.drag_line = ('h', i); return
            ...
            if col != -1 and row != -1:
                key = f"{row},{col}"
                if key in data['disabled']: data['disabled'].remove(key)
                else: data['disabled'].append(key)
    ...
    def mouseMoveEvent(self, event):
        if self.drag_line and self.page_item:
            ...
            if axis == 'v': data['v_lines'][idx] = max(0.01, min(0.99, pos.x() / w))
            else: data['h_lines'][idx] = max(0.01, min(0.99, pos.y() / h))
            self.viewport().update()
            return
        super().mouseMoveEvent(event)

    def mouseReleaseEvent(self, event): self.drag_line = None
    def wheelEvent(self, event: QWheelEvent): pass
...
class CropperPreviewWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.btn_zoom_out = QPushButton("➖ 缩小"); self.btn_zoom_fit = QPushButton("🔲 适应窗口"); self.btn_zoom_in = QPushButton("➕ 放大")
        self.btn_prev = QPushButton("◀ 上一页"); self.btn_next = QPushButton("下一页 ▶")
        self.entry_page = QLineEdit(); self.entry_page.setFixedWidth(60); self.entry_page.setAlignment(Qt.AlignCenter)
        ...
        self.btn_prev.setStyleSheet(nav_btn_style); self.btn_next.setStyleSheet(nav_btn_style)
        self.btn_zoom_out.setStyleSheet(zoom_btn_style); self.btn_zoom_fit.setStyleSheet(zoom_btn_style); self.btn_zoom_in.setStyleSheet(zoom_btn_style)
        ...
        self.nav_layout.addWidget(self.btn_zoom_out); self.nav_layout.addWidget(self.btn_zoom_fit); self.nav_layout.addWidget(self.btn_zoom_in)
        self.nav_layout.addStretch(1)
        self.nav_layout.addWidget(self.btn_prev); self.nav_layout.addWidget(QLabel("当前第"))
        self.nav_layout.addWidget(self.entry_page); self.nav_layout.addWidget(self.lbl_total)
        self.nav_layout.addWidget(self.btn_next); self.nav_layout.addStretch(1)
        self.layout.addLayout(self.nav_layout)

        self.btn_prev.clicked.connect(self._go_prev); self.btn_next.clicked.connect(self._go_next)
        self.entry_page.returnPressed.connect(self._jump_page); self.view.pageChanged.connect(self._on_page_changed)
        self.btn_zoom_in.clicked.connect(self._zoom_in); self.btn_zoom_out.clicked.connect(self._zoom_out); self.btn_zoom_fit.clicked.connect(self._zoom_fit)

    @property
    def page_data_dict(self): return self.view.page_data_dict
    @property
    def current_page(self): return self.view.current_page

    def load_pdf(self, doc, target_page=0, data_dict=None):
        self.view.load_pdf(doc, target_page, data_dict); self._zoom_fit()

    def _go_prev(self):
        if self.view.pdf_doc and self.view.current_page > 0: self.view.show_page(self.view.current_page - 1)

    def _go_next(self):
        if self.view.pdf_doc and self.view.current_page < len(self.view.pdf_doc) - 1: self.view.show_page(self.view.current_page + 1)

    def _jump_page(self):
        if not self.view.pdf_doc: return
        try:
            p = int(self.entry_page.text()) - 1
            if 0 <= p < len(self.view.pdf_doc): self.view.show_page(p)
            else: self.entry_page.setText(str(self.view.current_page + 1))
        except ValueError: self.entry_page.setText(str(self.view.current_page + 1))
    ...
    def _zoom_in(self):
        ...
        self.view.zoom_factor *= 1.15; self.view.scale(1.15, 1.15)

    def _zoom_out(self):
        if not self.view.page_item: return
        self.view.zoom_factor /= 1.15; self.view.scale(1 / 1.15, 1 / 1.15)
    ...
    def _zoom_fit(self):
        ...
        self.view.resetTransform(); self.view.zoom_factor = ratio; self.view.scale(ratio, ratio)
...
class CropperWidget(QWidget):
    ...
    def init_ui(self):
        ...
        top_widget = QWidget(); top_layout = QHBoxLayout(top_widget); top_layout.setContentsMargins(0, 0, 0, 0)
        box_left = QGroupBox("1. 载入图片与 PDF 混合列阵")
        l_left = QVBoxLayout()
        self.file_manager = FileListManagerWidget(accept_exts=['.pdf', '.jpg', '.png', '.jpeg', '.bmp'], title_desc="Files")
        ...
        box_left.setLayout(l_left); top_layout.addWidget(box_left, 1)
        ...
        l_right.addWidget(btn_detect); l_right.addWidget(btn_preview)

        tool_frame = QGroupBox("单页手工切线工具 (微调状态可用)")
        tool_l = QHBoxLayout()
        btn_add_v = QPushButton("➕ 加竖线"); btn_add_v.clicked.connect(lambda: self.preview_view.view.add_v_line())
        btn_add_h = QPushButton("➕ 加横线"); btn_add_h.clicked.connect(lambda: self.preview_view.view.add_h_line())
        btn_reset = QPushButton("↺ 重置清空"); btn_reset.clicked.connect(lambda: self.preview_view.view.reset_lines())
        tool_l.addWidget(btn_add_v); tool_l.addWidget(btn_add_h); tool_l.addWidget(btn_reset)
        tool_frame.setLayout(tool_l); l_right.addWidget(tool_frame); l_right.addStretch(1)
        ...
        hz_exp.addWidget(btn_exp_img); hz_exp.addWidget(btn_exp_pdf)
        l_right.addLayout(hz_exp)

        self.progress_bar = QProgressBar()
        l_right.addWidget(self.progress_bar)

        box_right.setLayout(l_right); top_layout.addWidget(box_right, 1)
        ...
        bottom_layout.addWidget(self.btn_confirm_pos); bottom_layout.addWidget(self.preview_view)

        splitter.addWidget(bottom_widget); splitter.setSizes([400, 600])
    ...
    def enter_setting_mode_from_dialog(self, seg_data, idx, dialog):
        self.current_idx = idx; self.dialog = dialog
    ...
    def confirm_crop_position(self):
        ...
        self.segments[self.current_idx]['pos_pct'] = new_cfg; self.segments[self.current_idx]['pos_set'] = True
        for p in self.segments[self.current_idx]['pages']: self.page_configs[p] = copy.deepcopy(new_cfg)
        self.btn_confirm_pos.hide(); self.dialog.refresh_table(); self.dialog.show()
    ...
    def enter_final_preview(self):
        ...
        QMessageBox.information(self, "高级微调模式", "已进入终极预览。\n您可以滚动翻页查看每一页；\n左键拖拽红线，右键红线删除，点击格子剔除区域！")
