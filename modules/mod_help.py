...
class HelpWidget(QWidget):
    ...
    def generate_help_text(self):
        ...
        html_content = f"""
                ...
                <body>
                    <div class="header">
                        <span class="studio-name">📚 PDF 聚合工作站 V4.0.0 - 操作指南</span>
                    </div>
                    ...
                    <ul>
                        <li><b>添加图章：</b>支持 PNG、JPG、JPEG 和矢量 PDF。多页 PDF 会先选择具体页码；预览仅使用缩略图，最终导出仍保留矢量文字与线条。</li>
                        ...
                        <li><b>画质与压缩控制：</b>图章像素按实际放置毫米尺寸与 DPI 计算，不会按原始大图尺寸嵌入，也不会放大低分辨率素材。同一图章会复用 PDF 图像对象。</li>
                        <li><b>GS 二次压缩：</b>支持 /screen、/ebook、/printer。错误提示会显示具体文件、退出码与 GS 原因；300 DPI 打印档会保留 Separation、DeviceN、PANTONE 名称和 ICC 资源，专色安全检查失败时不会交付错误结果。</li>
                        <li><b>🛡️ 防篡改数字签名锁：</b>勾选【附加防篡改保护锁】后，系统会：
                            <ul>
                                <li>使用您提供的 .pfx 数字证书，在指定图章位置创建合法签名域。</li>
                                <li>使用 pyHanko 认证签名和 DocMDP 禁止更改策略；执行前会验证证书文件、密码、私钥和有效期。</li>
                    ...
                    <ul>
                        ...
                        <li><b>重复裁剪：</b>导出的 PNG 会写入有效 DPI，再次导入时按物理尺寸建立页面。可连续多轮裁剪，不会每轮放大四倍。</li>
                    </ul>

                    <h2>🗜️ 模块 4：PDF 与图片强力压缩</h2>
                    <p>提供专业的媒体文件体积优化方案。</p>
                    <ul>
                        <li><b>PDF 强力压缩：</b>基于内置 Ghostscript。Screen 为 72 DPI，Ebook 为 150 DPI，Printer 为 300 DPI；Printer 档启用专色保护，避免 PANTONE 色变灰。</li>
                    ...
                    <ul>
                        ...
                        <li><b>PDF 交叉合并：</b>按 A1、B1、A2、B2 顺序合并，可预留空白页、倒序 B 文件并批量旋转两侧页面。</li>
                        ...
                        <li><b>PDF 转图片型 PDF：</b>单文件直接另存；多个 PDF 会选择目录并分别生成“原文件名_图片型.pdf”，保留各自页数、页面尺寸、顺序和可恢复书签。</li>
                        <li><b>PDF 批量导出图片：</b>将 PDF 每一页导出为高清 JPG 或 PNG，支持开启透明通道（仅PNG）。</li>
                    </ul>
                    <h2>📐 模块 6：线稿智能转 DXF</h2>
                    <p>基于顶级计算机视觉引擎，提供三种核心转换策略，满足从插画到建筑立面的全方位矢量化需求。</p>
                    <ul>
                        <li><b>策略1（插画级双线轮廓）：</b>完美还原笔画粗细，适合图形、图标、Logo等需要闭合面轮廓的图像。</li>
                        <li><b>策略2（手绘级单线剥离）：</b>通过形态学细化与回路截断算法，将具有宽度的手绘线稿抽离为一像素绝对居中的自由多段线。</li>
                        <li><b>策略3（建筑级正交捕捉）：</b>专为 AI 生成建筑方案与硬表面图纸打造。独有“共线融合”与“正交拉直”黑科技。能智能保留门窗的曲线，同时将歪斜的建筑线段绝对拉平为 0°/90°，极大减少后期 CAD 修正工作量。</li>
                    </ul>
                    
                    <h2>🖼️ 模块 7 (全新)：批量一对一加图</h2>
                    <p>区别于全局统一加盖的“图章工具”，此模块专为每个 PDF 插入各自独立的图片或矢量 PDF 素材。</p>
                    <ul>
                        <li><b>商业级智能配对：</b>分别导入 PDF 列表与图片列表。支持“按相同文件名称智能配对”或“按上下排序强制配对”。</li>
                        <li><b>全页覆盖或首页限定：</b>可自由选择是将对应图片插入到目标 PDF 的每一页中，还是仅插入首页。</li>
                        <li><b>排版逻辑高度复用：</b>全面继承“智能按图纸尺寸分段”与“终极自由微调预览”两大核心杀手锏。统一设置某尺寸图纸的几何占位位置，同时允许在微调模式下，对极个别出现偏差的图片进行单独拖拽矫正！</li>
                        <li><b>全能导出与瘦身：</b>支持合并长图纸、按原文件名批量导出、或拆分为单页独立输出。底层无缝接轨 Ghostscript 强力压缩引擎。</li>
                    </ul>

                    <h2>📑 模块 8：PDF 页面组织与阅读</h2>
                    <ul>
                        <li><b>页面组织：</b>所有页面以响应式缩略图平铺，可调整缩略图大小、拖动排序、多选、全选、删除并撤销或重做。</li>
                        <li><b>插入与旋转：</b>可把外部 PDF 拖到指定位置插入，支持旋转选中页面或全部页面。</li>
                        <li><b>保存与提取：</b>默认另存为副本，保留矢量、文字、链接、批注和仍有效的书签；选中多页可按当前顺序提取为一个 PDF。</li>
                        <li><b>阅读模式：</b>双击缩略图进入阅读，支持翻页、页码跳转、缩放、适合窗口；F11 全屏，Esc 退出。</li>
                        <li><b>签名提示：</b>编辑已签名 PDF 会使原数字签名失效，模块会在载入时明确提醒。</li>
                    </ul>
