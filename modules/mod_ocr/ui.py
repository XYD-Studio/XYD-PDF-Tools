...
class OCRExtractorWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.table.setToolTip("💡 提示：双击单元格可修改文本，左右拖动表头可调整顺序，双击表头可修改字段名称")
    ...
    def sync_data_from_ui(self):
        if self.table.columnCount() <= 1 or self.table.rowCount() == 0: return
        ...
        for v_idx in range(col_count):
            ...
            if l_idx == 0: continue
            header_item = self.table.horizontalHeaderItem(l_idx)
            if not header_item: continue
            fname = header_item.text()
            for old_f in self.fields_config:
                if old_f['name'] == fname:
        ...
        if len(new_fields) == len(self.fields_config):
            ...
            for r in range(self.table.rowCount()):
                ...
                for v_idx in range(col_count):
                    l_idx = header.logicalIndex(v_idx)
                    if l_idx == 0: continue
                    fname = self.table.horizontalHeaderItem(l_idx).text()
                    cell_item = self.table.item(r, l_idx)
                    row_dict[fname] = cell_item.text() if cell_item else ""
                new_ext_data.append(row_dict)
    ...
    def rename_column_header(self, logical_index):
        ...
        self.sync_data_from_ui()
        old_name = self.table.horizontalHeaderItem(logical_index).text()
    ...
    def export_config(self):
        ...
        out_data = {"fields_config": self.fields_config, "templates": templates}
    ...
    def detect_segments(self):
        ...
        if not self.segments: self.segments = detect_smart_segments(self.pdf_doc)
        ...
        for seg in self.segments:
            ...
            for k in existing_keys:
                if k not in ocr_fields: del current_boxes[k]
            ...
            for p in seg['pages']: self.page_configs[p] = dict(current_boxes)
    ...
    def enter_setting_mode_from_dialog(self, seg_data, idx, dialog):
        ...
        self.preview_view.set_nav_locked(True)  # 💡 防误操作锁死
    ...
    def confirm_box_position(self):
        ...
        self.preview_view.set_nav_locked(False)  # 💡 解锁
        self.btn_confirm.hide()
        self.dialog.refresh_table()
        self.dialog.show()
    ...
    def start_ocr(self):
        ...
        self.preview_view.save_current_page_state()
        # 💡 [修复核心：更新而不是覆盖数据，保证没进终极预览的其他页数据不丢失]
    ...
    def merge_json_files(self):
        ...
        try:
            ...
            self.extracted_data.extend(merged_list)
            self.refresh_table_data()
