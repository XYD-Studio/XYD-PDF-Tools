...
class ResizableRectItem(QGraphicsRectItem):
    ...
    def hoverMoveEvent(self, event):
        rect = self.rect()
        self.setCursor(Qt.SizeFDiagCursor if event.pos().x() > rect.right() - 15 and event.pos().y() > rect.bottom() - 15 else Qt.SizeAllCursor)
...
class ResizableStampItem(QGraphicsRectItem):
    def __init__(self, stamp_info, view):
        ...
        try:
            with open(stamp_info.get('path', ''), 'rb') as f: self.orig_pixmap.loadFromData(f.read())
        except Exception:
            self.orig_pixmap = QPixmap(int(w_scene), int(h_scene)); self.orig_pixmap.fill(Qt.lightGray)
    ...
    def paint(self, painter, option, widget):
        ...
        if not self.orig_pixmap.isNull():
            scaled_pixmap = self.orig_pixmap.scaled(int(rect.width()), int(rect.height()), Qt.IgnoreAspectRatio, Qt.SmoothTransformation)
            painter.drawPixmap(rect.toRect(), scaled_pixmap)
        if self.isSelected() or self.isUnderMouse():
            painter.setPen(QPen(Qt.red, 2, Qt.DashLine)); painter.setBrush(Qt.NoBrush); painter.drawRect(rect)
            painter.setBrush(Qt.red); painter.drawRect(int(rect.right() - 10), int(rect.bottom() - 10), 10, 10)
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            if event.modifiers() == Qt.AltModifier:
                self.view.duplicate_stamp(self.stamp_info); event.accept(); return
            rect = self.rect()
            if event.pos().x() > rect.right() - 15 and event.pos().y() > rect.bottom() - 15:
                self.resizing = True; event.accept(); return
        super().mousePressEvent(event)
    ...
    def contextMenuEvent(self, event):
        ...
        if action == action_copy: self.view.duplicate_stamp(self.stamp_info)
        elif action == action_replace:
            file_path, _ = QFileDialog.getOpenFileName(None, "选择要替换的图片", "", "Images (*.png *.jpg *.jpeg *.bmp)")
            if file_path:
                ...
                if not new_pixmap.isNull():
                    ...
                    if new_pixmap.width() >= new_pixmap.height(): final_w, final_h = max_dim, max_dim / new_ratio
                    else: final_h, final_w = max_dim, max_dim * new_ratio
                    self.stamp_info.update({'path': file_path, 'w': final_w, 'h': final_h, 'name': os.path.basename(file_path)})
                    with open(file_path, 'rb') as f: self.orig_pixmap.loadFromData(f.read())
            ...
            self.lock_ratio = not self.lock_ratio; self.stamp_info['lock_ratio'] = self.lock_ratio; self.update()
        elif action == action_rot_90:
            new_angle = (self.rotation() + 90) % 360; self.setRotation(new_angle); self.stamp_info['angle'] = new_angle
...
class _PDFGraphicsView(QGraphicsView):
    ...
    def __init__(self, parent=None):
        ...
        self.page_item = None; self.mode = 'view'
        self.page_data_dict = {}; self.dynamic_items = {}
        self.color_palette = [QColor(255, 0, 0), QColor(0, 0, 255), QColor(0, 200, 0), QColor(255, 140, 0), QColor(128, 0, 128), QColor(0, 150, 255)]

    def load_pdf(self, doc, target_page=0, mode='view', data_dict=None, **kwargs):
        self.pdf_doc = doc; self.mode = mode; self.page_data_dict = data_dict if data_dict is not None else {}
        self.current_page = -1; self.page_item = None; self.dynamic_items.clear()
    ...
    def show_page(self, page_num):
        ...
        self.scene.clear(); self.dynamic_items.clear()
        ...
        self.pageChanged.emit(self.current_page); self.scene.update()
    ...
    def save_current_page_state(self):
        ...
        if self.mode == 'stamp_final':
            ...
            for field_name, item in self.dynamic_items.items():
                new_pos[field_name] = (item.pos().x() / RENDER_SCALE, item.pos().y() / RENDER_SCALE, item.rect().width() / RENDER_SCALE, item.rect().height() / RENDER_SCALE)
    ...
    def _draw_stamp(self, stamp_info):
        try:
            ...
            item.setPos(float(stamp_info.get('pdf_x', 100)) * RENDER_SCALE, float(stamp_info.get('pdf_y', 100)) * RENDER_SCALE)
    ...
    def _draw_ocr_boxes(self, pos_dict):
        try:
            ...
            for i, (field_name, data) in enumerate(pos_dict.items()):
                ...
                pdf_x, pdf_y, pdf_w, pdf_h = map(float, data[:4]) if isinstance(data, (list, tuple)) and len(data) >= 4 else (100.0, 100.0, 150.0, 40.0)
                item = ResizableRectItem(max(20.0, pdf_w * RENDER_SCALE), max(20.0, pdf_h * RENDER_SCALE), str(field_name), color)
    ...
    def delete_stamp(self, stamp_id):
        ...
        if item:
            self.scene.removeItem(item); del self.dynamic_items[stamp_id]
            if self.current_page in self.page_data_dict:
                self.page_data_dict[self.current_page] = [s for s in self.page_data_dict[self.current_page] if s.get('id') != stamp_id]

    def wheelEvent(self, event: QWheelEvent):
        if event.modifiers() == Qt.ControlModifier:
            zoom = 1.15 if event.angleDelta().y() > 0 else 1 / 1.15
            self.zoom_factor *= zoom; self.scale(zoom, zoom)
        else:
...
class PDFGraphicsView(QWidget):
    def __init__(self, parent=None):
        ...
        self.view = _PDFGraphicsView(); self.layout.addWidget(self.view, 1)
        ...
        self.nav_layout.addWidget(self.btn_prev); self.nav_layout.addWidget(QLabel("当前第")); self.nav_layout.addWidget(self.entry_page)
        self.nav_layout.addWidget(self.lbl_total); self.nav_layout.addWidget(self.btn_next); self.nav_layout.addStretch(1)
        self.layout.addLayout(self.nav_layout)
        self.btn_prev.clicked.connect(self._go_prev); self.btn_next.clicked.connect(self._go_next); self.entry_page.returnPressed.connect(self._jump_page)
        self.view.pageChanged.connect(self._on_page_changed)
        self.btn_zoom_in.clicked.connect(lambda: self._do_zoom(1.2)); self.btn_zoom_out.clicked.connect(lambda: self._do_zoom(1/1.2)); self.btn_zoom_fit.clicked.connect(self._zoom_fit)
    ...
    def load_pdf(self, doc, target_page=0, mode='view', data_dict=None, **kwargs):
        self.view.load_pdf(doc, target_page, mode, data_dict, **kwargs); self._zoom_fit()

    def save_current_page_state(self): self.view.save_current_page_state()
    @property
    def page_data_dict(self): return self.view.page_data_dict
    @page_data_dict.setter
    def page_data_dict(self, val): self.view.page_data_dict = val
    @property
